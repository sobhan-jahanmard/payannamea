...
def run(context: dict[str, Any], services: Any) -> None:
    ...
    script = Path(__file__).resolve().parent.parent / "utils" / "finalize_persian_pagination.ps1"
    command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-STA", "-File", str(script), "-Path", str(docx)]
    ...
    write_json(services.workspace / "reports" / "stage_checks" / "persian_pagination.json", {"passed": True, "pages": int(match.group(1)), "sections": int(match.group(2)), "mode": "dynamic_persian_page_field"})
