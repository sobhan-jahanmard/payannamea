...
import { ConsultationLeadSchema, UserSchema } from "./db/entities";
import { ApiError } from "./http";
import { updateConsultationLead } from "./leads";
...
export const phoneFollowupUpdateSchema = z.union([
  z.object({
  ...
  z.object({
    ...
    message: "A lead change is required"
  })
]);
...
export async function findPhoneFollowup(rawQuery: unknown) {
  ...
  const [user, lead] = await Promise.all([
    dataSource.getRepository(UserSchema)
      ...
      .groupBy("user.id")
      .getRawAndEntities(),
    dataSource.getRepository(ConsultationLeadSchema).findOneBy({ phone })
  ]);
...
export async function updatePhoneFollowup(rawInput: unknown) {
  ...
  if (input.target === "user") {
    ...
    return { target: "user" as const, record: updated };
  }
  return {
    target: "lead" as const,
    record: await updateConsultationLead({ id: input.id, status: input.status, admin_note: input.admin_note })
  };
}
