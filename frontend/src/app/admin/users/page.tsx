...
import type { AdminUsersResponse, UserFollowupStatus } from "../../../types/api";
...
function AdminUsersPanel() {
  ...
  async function load(nextPage = page, statusOverride?: UserFollowupStatus | "") {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "بارگذاری کاربران ناموفق بود");
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="mx-auto grid w-full max-w-7xl gap-5 px-4 py-6 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">فهرست کاربران و وضعیت پیگیری آن‌ها توسط ادمین</p>
        </div>
      ...
      <section className="tool-surface grid gap-4 p-5 sm:grid-cols-[1fr_220px_auto] sm:items-end">
        <div className="space-y-2"><Label>جست‌وجوی کاربر یا یادداشت</Label><Input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void load(1); }} placeholder="نام، ایمیل، شماره موبایل یا یادداشت..." /></div>
        <div className="space-y-2"><Label>وضعیت</Label><Select value={status} onChange={(e) => setStatus(e.target.value as UserFollowupStatus | "")}><option value="">همه وضعیت‌ها</option>{(Object.keys(statusLabels) as UserFollowupStatus[]).map((key) => <option key={key} value={key}>{statusLabels[key]}</option>)}</Select></div>
        ...
        <table className="w-full min-w-[1400px] text-right text-sm">
          ...
          <tbody>{data?.users.map((user) => {
            ...
            return <tr key={user.id} className="border-b border-border/70 align-top">
              ...
              <td className="p-3"><Textarea className="min-h-20 resize-y" value={draft.admin_note} maxLength={2000} disabled={isSaving} placeholder="نتیجه تماس یا توضیحات پیگیری..." onChange={(e) => setDrafts((current) => ({ ...current, [user.id]: { ...draft, admin_note: e.target.value } }))} /></td>
              <td className="p-3"><Button type="button" size="sm" loading={isSaving} disabled={updatingId !== null && !isSaving} onClick={() => void saveUser(user.id)}><Save className="h-4 w-4" />ذخیره</Button></td>
            </tr>;
          })}</tbody>
        </table>
        {!data?.users.length ? <p className="p-6 text-center text-sm text-muted-foreground">کاربری پیدا نشد.</p> : null}
...
export default function AdminUsersPage() {
  return <AuthGate adminOnly><AdminUsersPanel /></AuthGate>;
}
