...
import { ChevronLeft, ChevronRight, RefreshCcw, Save, Search } from "lucide-react";
...
import { getAdminConsultationLeads, updateConsultationLead } from "../../../lib/api";
...
function LeadsPanel() {
  ...
  const [drafts, setDrafts] = useState<Record<string, { status: ConsultationLeadStatus; admin_note: string }>>({});
  ...
  async function load(nextPage = page, statusOverride?: ConsultationLeadStatus | "") {
    ...
    setError(null);
    try {
      const response = await getAdminConsultationLeads({
      ...
      setData(response);
      setDrafts(Object.fromEntries(response.leads.map((lead) => [
        lead.id,
  ...
  async function saveLead(id: string) {
    ...
    setError(null);
    try {
      const updated = await updateConsultationLead(id, draft);
      ...
      setError(updateError instanceof Error ? updateError.message : "ذخیره تغییرات ناموفق بود");
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  return (
    <main className="mx-auto grid w-full max-w-7xl gap-5 px-4 py-6 lg:px-8">
      ...
      <section className="tool-surface grid gap-4 p-5 sm:grid-cols-[1fr_220px_auto] sm:items-end">
        <div className="space-y-2"><Label>جست‌وجوی شماره یا یادداشت</Label><Input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void load(1); }} placeholder="شماره موبایل یا متن یادداشت..." /></div>
      ...
      <section className="tool-surface overflow-x-auto p-5">
        <table className="w-full min-w-[1100px] text-right text-sm">
          <thead className="border-b border-border text-xs text-muted-foreground"><tr><th className="p-3">شماره موبایل</th><th className="p-3">اولین درخواست</th><th className="p-3">آخرین درخواست</th><th className="p-3">تعداد</th><th className="w-48 p-3">وضعیت پیگیری</th><th className="w-80 p-3">یادداشت ادمین</th><th className="p-3">عملیات</th></tr></thead>
          <tbody>{data?.leads.map((lead) => {
            const draft = drafts[lead.id] ?? { status: lead.status, admin_note: lead.admin_note ?? "" };
            const isSaving = updatingId === lead.id;
            return <tr key={lead.id} className="border-b border-border/70 align-top">
              <td className="ltr p-3 text-left font-medium">{lead.phone}</td>
              <td className="whitespace-nowrap p-3">{formatDateTime(lead.created_at)}</td>
              <td className="whitespace-nowrap p-3">{formatDateTime(lead.last_requested_at)}</td>
              <td className="p-3">{number(lead.request_count)}</td>
              <td className="p-3"><Select value={draft.status} disabled={isSaving} onChange={(e) => setDrafts((current) => ({ ...current, [lead.id]: { ...draft, status: e.target.value as ConsultationLeadStatus } }))}>{(Object.keys(statusLabels) as ConsultationLeadStatus[]).map((key) => <option key={key} value={key}>{statusLabels[key]}</option>)}</Select></td>
              <td className="p-3"><Textarea className="min-h-20 resize-y" value={draft.admin_note} maxLength={2000} disabled={isSaving} placeholder="نتیجه تماس یا توضیحات پیگیری..." onChange={(e) => setDrafts((current) => ({ ...current, [lead.id]: { ...draft, admin_note: e.target.value } }))} /></td>
              <td className="p-3"><Button type="button" size="sm" loading={isSaving} disabled={updatingId !== null && !isSaving} onClick={() => void saveLead(lead.id)}><Save className="h-4 w-4" />ذخیره</Button></td>
            </tr>;
          })}</tbody>
        </table>
