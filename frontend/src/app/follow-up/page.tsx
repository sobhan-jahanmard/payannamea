...
import type { PhoneFollowupResponse, UserFollowupStatus } from "../../types/api";
...
function FollowUpPanel() {
  ...
  if (!canFollowUp) {
    return <main className="mx-auto w-full max-w-3xl px-4 py-10 lg:px-8"><section className="tool-surface p-6">دسترسی به پیگیری شماره ندارید.</section></main>;
  }
...
function FollowupCard({ title, exists, meta, draft, saving, onChange, onSave }: { title: string; exists: boolean; meta?: string; draft: { status: UserFollowupStatus; note: string } | null; saving: boolean; onChange: (value: { status: UserFollowupStatus; note: string } | null) => void; onSave: () => void }) {
  if (!exists || !draft) return <section className="tool-surface p-5"><h2 className="font-semibold">{title}</h2><p className="mt-2 text-sm text-muted-foreground">رکوردی با این شماره پیدا نشد.</p></section>;
  return <section className="tool-surface grid gap-4 p-5"><div><h2 className="font-semibold">{title}</h2><p className="mt-1 text-sm text-muted-foreground">{meta}</p></div><div className="grid gap-4 sm:grid-cols-[220px_1fr_auto] sm:items-end"><div className="space-y-2"><Label>وضعیت پیگیری</Label><Select value={draft.status} disabled={saving} onChange={(event) => onChange({ ...draft, status: event.target.value as UserFollowupStatus })}>{(Object.keys(statusLabels) as UserFollowupStatus[]).map((key) => <option key={key} value={key}>{statusLabels[key]}</option>)}</Select></div><div className="space-y-2"><Label>یادداشت پیگیری</Label><Textarea value={draft.note} disabled={saving} maxLength={2000} className="min-h-20 resize-y" onChange={(event) => onChange({ ...draft, note: event.target.value })} placeholder="نتیجه تماس یا توضیحات..." /></div><Button type="button" loading={saving} onClick={onSave}><Save className="h-4 w-4" />ذخیره</Button></div></section>;
}

export default function FollowUpPage() {
  return <AuthGate><FollowUpPanel /></AuthGate>;
}
